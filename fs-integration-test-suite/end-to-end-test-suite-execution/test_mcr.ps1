...
if ($Help) {
    Write-Output "Usage: .\test_mcr.ps1 -InputDir <path> -OutputDir <path> [-MavenOpts <opts>]"
    exit 0
}
...
Write-Output "[INFO] Executing Accelerator Scenarios with Manual Client Registration"
Invoke-TestSuite -SuitePath (Join-Path $gatewayIntegrationTestHome "manual.client.registration") -OutputPath (Join-Path $OutputDir "scenarios/gateway-tests/manual.client.registration") -MavenArgs @("clean", "install", "-Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn", "-fae", "-B", "-f", "pom.xml")
...
Invoke-TestSuite -SuitePath (Join-Path $gatewayIntegrationTestHome "payments") -OutputPath (Join-Path $OutputDir "scenarios/gateway-tests/payments") -MavenArgs @("clean", "install", "-DdcrEnabled=false", "-Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn", "-fae", "-B", "-f", "pom.xml")
...
Invoke-TestSuite -SuitePath (Join-Path $gatewayIntegrationTestHome "token") -OutputPath (Join-Path $OutputDir "scenarios/gateway-tests/token") -MavenArgs @("clean", "install", "-Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn", "-fae", "-B", "-f", "pom.xml")
$nonRegSuitePath = Join-Path $gatewayIntegrationTestHome "non.regulatory.scenarios"
Invoke-TestSuite -SuitePath $nonRegSuitePath -OutputPath (Join-Path $OutputDir "scenarios/gateway-tests/non.regulatory.scenarios") -MavenArgs @("clean", "install", "-Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn", "-fae", "-B", "-f", "pom.xml")

Write-Output "[INFO] End of Accelerator Scenarios with Manual Client Registration"
Write-Output "[INFO] End of Gateway tests"
Copy-DirectoriesByName -SourceRoot $nonRegSuitePath -DirectoryName "aggregate-surefire-report" -DestinationRoot (Join-Path $OutputDir "scenarios")
